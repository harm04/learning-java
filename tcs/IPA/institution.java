...
class Institution {
    ...
    void setNoOfStudentsPlaced(int noOfStudentsPlaced) {
        this.noOfStudentsPlaced = noOfStudentsPlaced;
    }
...
class InstitutionSolution {
    public static int FindNumClearancedByLoc(Institution[] inst, String location) {
        int sum = 0;
        for (int i = 0; i < inst.length; i++) {
            if (inst[i].getLocation().equalsIgnoreCase(location)) {
    ...
    public static Institution UpdateInstitutionGrade(Institution[] inst, String institutionName) {
        for (int i = 0; i < inst.length; i++) {
            if (inst[i].getInstitutionName().equalsIgnoreCase(institutionName)) {
                int rating = (inst[i].getNoOfStudentsPlaced() * 100) / inst[i].getNoOfStudentsCleared();
                if (rating >= 80) {
                    inst[i].setGrade("A");
                } else {
                    inst[i].setGrade("B");
